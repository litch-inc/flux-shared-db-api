...
async function getMyIp(OpNodes, retries = 1) {
  ...
  for (let i = 0; i < OpNodes.length && i < 5; i++) {
    let tempIp = await FluxFunction.getMyIp(OpNodes[i].ip, 33950);
    let j = 1;
    while (tempIp === null && j < 6) {
      console.log(`node ${OpNodes[i].ip} not responding to api port ${33950}, retrying ${j}/5...`);
      await timer.setTimeout(2000);
      tempIp = await FluxFunction.getMyIp(OpNodes[i].ip, 33950);
      j++;
    }
...
async function testFluxAPI3() {
  while(true) {
    ipList = await FluxFunction.getApplicationIP('wordpress1678039648505');
    const ips = [];
    for (let i = 0; i < ipList.length; i++) {
      ips.push(ipList[i].ip);
    }
    Logger.apiLogger.info(ips);
